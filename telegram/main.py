...
def help(message):
    helping_message = "Привет! Для начала взаимодействия с нашим ботом ты должен выбрать валюту(пока доступны операции только с рублем), банки и биржи, на которых ты будешь торговать. Запомни: чем больше выбранных банков и бирж, тем больше выгодных вариантов. Бот почти моментально выдаёт связки, но помни, что реализация связок занимает время, поэтому завари чай и возьми печенек, на торговлю нужно время. Если что-то сломалось или тебе что-то не понятно, то пиши @vazy1 за помощью. version 0.1.1"
